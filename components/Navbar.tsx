...
export default function Navbar() {
  ...
  return (
    <nav
      className={`fixed top-0 w-full z-50 backdrop-blur-xl border-b border-black/10 transition-all duration-300 glass-effect-light ${isScrolled
    ...
    >
      <div className="flex justify-between items-center px-margin-mobile md:px-margin-desktop py-3 max-w-container-max mx-auto">
        ...
        <div className="hidden md:flex items-center">
          <Link
            href="/contact"
            className="bg-[#4b5a20] text-[#bfd189] px-7 py-3 rounded-xl font-bold font-mono-code text-xs tracking-wider uppercase hover:bg-primary hover:text-[#283500] transition-all duration-300 active:scale-95 shadow-lg shadow-primary/10 inline-block"
          >
            Get Started
          </Link>
        </div>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden bg-white/95 border-b border-black/10 px-6 py-6 space-y-4">
          {navItems.map((item) => {
            const isActive =
              item.href === "/"
                ? pathname === "/"
            ...
            return (
              <Link
                ...
                className={`block font-mono-code text-sm tracking-wider uppercase py-2 transition-colors ${isActive
